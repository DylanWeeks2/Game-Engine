...
//-----------------------------------------------------------------------------------------------
double RandomNumberGenerator::RollRandomDoubleZeroToOne()
{
	/*unsigned int randomUInt = Get1dNoiseUint(m_position++, m_seed);
	constexpr unsigned int MAX_RANDOM_UINT = 0xFFFFFFFF;
	constexpr double ONE_OVER_RAND_MAX = 1.0 / double(MAX_RANDOM_UINT);
	...
	return Get1dNoiseZeroToOneDP(m_position++, m_seed);
}

//-----------------------------------------------------------------------------------------------
double RandomNumberGenerator::RollRandomDoubleInRange(double minInclusive, double maxInclusive)
{
	double range = maxInclusive - minInclusive;
	return (RollRandomDoubleZeroToOne() * range) + minInclusive;
}
