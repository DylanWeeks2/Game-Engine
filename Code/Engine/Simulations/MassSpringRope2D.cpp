...
void MassSpringRope2D::Render(std::vector<Vertex_PCU>& verts) const
{
	...
	for (int springIndex = 0; springIndex < m_springs.size(); springIndex++)
	{
		...
		if (spring != nullptr)
		{
			...
			if (m_isDebugMode)
			{
				AddVertsForDisc2D(verts, spring->m_pointA->m_position, m_ropeRadius * 3.0f, Rgba8::WHITE);
				AddVertsForDisc2D(verts, spring->m_pointB->m_position, m_ropeRadius * 3.0f, Rgba8::WHITE);
			}
		}
	}
}
