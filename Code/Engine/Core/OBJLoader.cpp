...
//-----------------------------------------------------------------------------------------------
void OBJLoader::Load(std::string filename, std::vector<Vertex_PNCU>& vertices, std::vector<int>& indices, Mat44& transform)
{
	Strings verts;
	...
	if (faces.size() > 0)
	{
		for (int faceIndex = 1; faceIndex <= indices.size(); faceIndex++)
		{
			if (faceIndex % 3 == 0)
			{
				...
				vertices.push_back(Vertex_PNCU(parsedVerts[indices[faceIndex - 3]], normal1, Rgba8(), textureCoords1));
				vertices.push_back(Vertex_PNCU(parsedVerts[indices[faceIndex - 2]], normal2, Rgba8(), textureCoords2));
				vertices.push_back(Vertex_PNCU(parsedVerts[indices[faceIndex - 1]], normal3, Rgba8(), textureCoords3));
	...
	else
	{
		for (int index = 1; index <= parsedVerts.size(); index++)
		{
			if (index % 3 == 0)
			{
				...
				vertices.push_back(Vertex_PNCU(parsedVerts[index - 3], normal, Rgba8(), Vec2()));
				vertices.push_back(Vertex_PNCU(parsedVerts[index - 2], normal, Rgba8(), Vec2()));
				vertices.push_back(Vertex_PNCU(parsedVerts[index - 1], normal, Rgba8(), Vec2()));
				index += 2;
			}
		}
	}
